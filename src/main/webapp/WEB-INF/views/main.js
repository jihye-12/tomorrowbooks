...
if(number == 0) { //추후 백엔드에서 쿼리로 가져오기 편하게 만들어 보았습니다.
    imglink.setAttribute('href', '/uncom-conveni.html');
    mainimg_cation.textContent = '불편한데 자꾸 가고 싶은 편의점이 있다!';
} else if(number == 1) {
    imglink.setAttribute('href', '/dalergoot.html');
    mainimg_cation.textContent = '잠들어야만 입장 가능한 꿈 백화점';
} else if(number == 2) {
    imglink.setAttribute('href', '/laSeine.html');
    mainimg_cation.textContent = '사망한 여인이 센 강에서 발견되었다.';
} else if(number == 3) {
    imglink.setAttribute('href', '/midnightlibrary.html');
    mainimg_cation.textContent = '죽기 바로 전에만 열리는 마법의 도서관';
} else {
    imglink.setAttribute('href', '/socrates.html');
